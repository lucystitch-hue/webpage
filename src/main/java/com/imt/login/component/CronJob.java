...
public class CronJob {
    ...
    public void sendNotificationJob() {
		...
		try {
			...
			//get host name from db with key code = 
  ...
  public void deleteAllFileCSV() throws IOException {
    ...
    @Scheduled(cron = "0 */3 * * * *")
    public void sendNotificationAAAAJob() {
		System.out.println("Starting cron job sendNotification AAAA");
		try {
			//get host name
			InetAddress localMachine = InetAddress.getLocalHost();
			String hostname = localMachine.getHostName();
			System.out.println("Hostname: " + hostname);
			//get host name from db with key code = 0
			Asys01 asys01 = asys01Repository.findAllByKeyCode();
			// check condition with host name
			if(asys01.getSys01Value().equals(hostname)){
				cronJobService.sendNotificationAAAA();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
